...
在 `/root/code/InternLM/chat/web_demo.py` 中，用`ctrl + F` 进行搜索找到`load_model()` 函数，修改如图所示对应的参数，该参数是载入模型的路径。

更换为Merge后存放参数的路径 `/root/personal_assistant/config/work_dirs/hf_merge`
