...
目前有两种途径获取书生大模型 API 服务：**社区 API** 和 **官方 API**，两者均兼容 OpenAI 和 Anthropic 协议。
...
| **Base URL** | `https://chat.intern-ai.org.cn/api/v1` | `https://chat.intern-ai.org.cn/api/v1` |
...
| **兼容性** | OpenAI / Anthropic 协议兼容 | OpenAI / Anthropic 协议兼容 |
...
两种 API 均兼容 OpenAI SDK 和 Anthropic 协议，只需修改 `base_url` 和 `api_key` 即可切换：
