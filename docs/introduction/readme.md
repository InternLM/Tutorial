# 书生大模型实战营 · 第七季

> 本文档 AI + 社区共建中

与书小生共赴 AGI4S 时代浪潮。

## 欢迎

欢迎来到书生大模型实战营第七季。这不是一门普通的在线课程——这是一次动手实践的旅程。

## 什么是 AGI4S

Slogan「与书小生共赴 AGI4S 时代浪潮」，**AGI4S** 是两部分的合体：

- **AGI（Artificial General Intelligence）** — 通用人工智能，追求"一个模型能解决多类问题"的能力
- **4S（for Science）** — 让 AGI 服务于科学发现，从数学/物理/化学到生物/地球/生命科学

合起来就是 **"把通用智能用到科学里去"**。这是实战营第七季围绕的大方向：既要让你能用 AGI 级能力（写代码、连接工具、多模态理解），也要让你学会如何让这些能力在真实科学与产业场景里落地、安全运行。

## 你将用 Claude Code 搞懂 MCP 和 Skills

**为什么以 Claude Code 为载体？**

AI 编程、MCP（让 AI 调外部工具的协议）、Skills（把工作流变成 AI 可复用能力）—— 这几样是 Anthropic 提出并沉淀下来的事实标准。从最原始、最权威的工具学起，之后无论换到哪个 AI 编程终端、哪个底层大模型，这套思维都通用。

## 你将做出什么

学完这些课程，你能带走四个真正能用的东西：

1. **一个可调用的大模型 API 服务** — 用 LMDeploy 量化部署，4GB 显存跑 8B 模型，对外提供 OpenAI 兼容接口
2. **一个能连接工具的 AI Agent** — 通过 MCP 协议让 AI 访问数据库、搜索引擎、文件系统，不再只能聊天
3. **一个安全运行的智能体** — 接入书安 Intern-Shannon 的硬件级沙箱、监管协议、风险评估，Agent 跑在可审计、可管控的闭环里
4. **一个在微信/Telegram 里运行的 AI 助手** — 用 OpenClaw 搭建，接入 Intern-S1-Pro 科学大模型，数据在你自己手里

## 为什么是第七季

书生大模型实战营已经走过六季。第七季做了几个重要变化：

- **课程方向从 3 个扩展到 7 个**，覆盖 AI 编程、AI 协议、多模态、工程部署、科学模型、智能体安全、AI 应用
- **全部课程免费开放**，不设解锁门槛，按兴趣选课
- **闯关式学习**，学完即练，练完即考，确保真正掌握
- **社区共建**，课程内容持续迭代，你的反馈直接改进教材

## 书生生态一览

书生大模型生态是一套完整的 AI 开源工具链，每一门课程都围绕生态中的核心产品展开：

| 产品 | 定位 | 对应课程 |
|------|------|---------|
| **Intern-S1-Pro** | 万亿参数科学多模态大模型 | Intern-S1-Pro 科学大模型 |
| **书安 Intern-Shannon** | 智能体安全操作系统 | 书安 Intern-Shannon 智能体安全 |
| **InternVL-U** | 统一多模态模型（理解+生成+编辑） | InternVL-U 统一多模态 |
| **InternSVG** | 矢量图形 AI 生成系统 | InternSVG 矢量图形 AI |
| **LMDeploy** | 高效推理部署工具 | LMDeploy 模型部署 |
| **InternLM** | 高性能语言大模型 | 贯穿多门课程 |

除了书生自研产品，我们还引入了业界优秀的开源工具：

| 工具 | 用途 | 对应课程 |
|------|------|---------|
| **Claude Code** | AI 编程终端 | Claude Code AI 编程 |
| **Skills** | AI 技能编排系统 | Skills 技能编排 |
| **MCP** | 模型上下文协议 | MCP 工具集成 |
| **A2A** | 多智能体协作协议 | Agent2Agent |
| **OpenClaw** | 多平台 AI 助手框架 | OpenClaw 多平台 AI 助手 |

## 七大方向

### AI 编程

用 AI 重新定义编程工作流。学完后你能用 AI 终端完成从需求到部署的全流程，还能编写自定义技能让 AI 按你的规范自动工作。

**课程：** Claude Code / Skills

### AI 协议

掌握 AI 世界的两大标准协议。MCP 让 AI 连接外部工具（数据库、API、文件系统），A2A 让多个 AI Agent 互相协作完成复杂任务。

**课程：** MCP / Agent2Agent

### 多模态

体验视觉 AI 的前沿能力。InternVL-U 一个模型同时做图像理解、生成和编辑；InternSVG 直接用 AI 生成可编辑的矢量图形代码。

**课程：** InternVL-U / InternSVG

### 工程部署

让大模型从实验室走进生产线。学会用 LMDeploy 的 TurboMind 引擎做量化部署，用更少的显存跑更大的模型，并搭建 OpenAI 兼容的 API 服务。

**课程：** LMDeploy

### 科学模型

探索 AI 在科学研究中的应用。Intern-S1-Pro 是万亿参数 MoE 架构的科学多模态大模型，在数学、物理、化学推理上达到前沿水平。

**课程：** Intern-S1-Pro

### 智能体安全

让智能体在产业规模化落地时依然安全可控。书安 Intern-Shannon 是一套智能体安全基础设施：硬件级 MicroVM 沙箱（A3S Box）、通用 Agent 监管协议（AHP）、三层决策风控（ClawSentry）、自护航技能包（SentrySkills）、多智能体风险评估（TrinityGuard）。

**课程：** 书安 Intern-Shannon

### AI 应用

在微信、Telegram 里搭建你自己的 AI 助手。OpenClaw 是开源的个人 AI 助手框架，支持多平台接入，可按部署方式控制数据边界。

**课程：** OpenClaw

## 闯关机制

每门课统一三步通关：

### 第一步：学教材

阅读课程文档，跟着代码示例动手操作。每门课的教材都经过精心设计，从原理讲起，到完整的可运行代码，再到实际的应用场景。

### 第二步：看视频

观看配套讲解视频，加深对关键概念的理解。视频不是教材的简单重复，而是从不同角度帮你建立直觉。

### 第三步：过考试

完成 5 道考核题（60 分及格）。题目侧重理解和应用，不考死记硬背。不限重考次数，取最高分。

**通关奖励：** 10 门课全部通关，颁发书生大模型实战营结业证书。

## 学习建议

### 新手路线（推荐零基础）

```
Intern-S1-Pro → Claude Code → Skills → MCP → LMDeploy → InternVL-U → InternSVG → A2A → 书安 → OpenClaw
```

先认识书生大模型核心能力，再学 AI 编程工具，最后学安全和应用落地。

### 工程师路线（有编程经验）

```
Claude Code → Skills → MCP → A2A → LMDeploy → 书安 → OpenClaw → InternVL-U → InternSVG → Intern-S1-Pro
```

先建立 AI 编程和工具连接能力，再学监管和应用，最后补齐多模态与科学模型。

### 研究者路线（关注科学应用）

```
Intern-S1-Pro → InternVL-U → InternSVG → LMDeploy → MCP → Claude Code → Skills → A2A → 书安 → OpenClaw
```

先掌握科学模型能力，再学工程化工具，最后学安全与应用落地。

### 安全与合规路线（企业 / 合规岗位）

```
书安 → MCP → Claude Code → A2A → OpenClaw → LMDeploy → Skills → Intern-S1-Pro → InternVL-U → InternSVG
```

先建立 Agent 安全治理能力（沙箱 / 监管 / 风险评估），再理解 Agent 技术栈，最后补齐模型和多模态。

### 自由探索

所有课程全部开放，不设顺序限制。按兴趣任选。

## 算力平台

**本次实战营将采用 昇腾 / 曦云 / A100 算力进行授课。**

三个平台均提供免费/低成本的开发环境，课程里的实操可以直接在上面跑：

| 平台 | 入口 | 说明 |
|------|------|------|
| **InternStudio 昇腾** | https://internstudio-ascend.intern-ai.org.cn | 华为昇腾 Atlas 800T A2，aarch64 + CANN 生态 |
| **Gitee AI 曦云** | https://ai.gitee.com/compute | 曦云 MetaX C500，x86_64 + MACA 生态 |
| **InternStudio A100** | https://studio.intern-ai.org.cn | NVIDIA A100 GPU，标准 CUDA 生态 |

注册后直接开机，预装 PyTorch / 常用库。每门课的「实操」子 Tab 都会分 昇腾 Atlas 800T A2、曦云 MetaX C500、A100 三个平台给出对应命令，你按自己手上的环境选。

没有自己的 GPU 也没关系，上述三个平台就够你完成全部课程。

## 前置要求

- 会基本的 Python 编程（变量、函数、包管理）
- 了解命令行操作（cd、ls、pip install）
- 不需要深度学习基础，课程从零讲起
- 如果 Python 和命令行还不太熟，可以先从 Claude Code 课开始，边用 AI 辅助边学

## 社区共建

这份教材是 AI + 社区共建的成果。学习过程中：

- **划词反馈**：在教材中选中有问题的文字，直接提交反馈，精确到行号
- **课程笔记**：记录你的实操过程和心得，沉淀为个人成长档案
- **课程建议**：对内容结构、章节编排提出改进建议

你的每一条反馈都会直接改进教材质量。参加实战营，与书小生共赴 AGI4S 时代浪潮。

## 开始学习

准备好了吗？从下面选一门课开始你的旅程：

1. [Intern-S1-Pro 科学大模型](../intern-s1-pro/readme.md) — 认识万亿参数科学大脑
2. [书安 Intern-Shannon 智能体安全](../intern-shannon/readme.md) — Agent 安全基础设施
3. [Claude Code AI 编程](../claude-code/readme.md) — 用 AI 终端写代码
4. [MCP 工具集成](../mcp/readme.md) — 让 AI 连接万物
5. [InternVL-U 统一多模态](../internvl-u/readme.md) — 一个模型搞定理解+生成+编辑
6. [LMDeploy 模型部署](../lmdeploy/readme.md) — 让大模型走进生产线
7. [Skills 技能编排](../skills/readme.md) — 让 AI 学会你的工作流
8. [Agent2Agent 多智能体协作](../agent2agent/readme.md) — AI Agent 之间的协作
9. [InternSVG 矢量图形 AI](../internsvg/readme.md) — 用 AI 画矢量图
10. [OpenClaw 多平台 AI 助手](../openclaw/readme.md) — 在微信/Telegram 里用 AI
