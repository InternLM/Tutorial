...
在开始学习之前，先看看 InternSVG 能做什么。以下三个成果均可通过 InternSVG-8B 模型一次生成初稿，通常只需少量微调即可用于项目。
...
| 16-24 GB | INT4 量化 | 质量几乎无损，推荐 |
| 10-16 GB | INT4 量化 + 减少并发 | 简单图形效果好，复杂图形略有下降 |
...
InternSVG 基于 InternVL3-8B 架构，可尝试在华为昇腾 Atlas 800T A2 上通过 torch_npu 适配运行。具体兼容性以官方支持矩阵和实测环境为准，以下为参考配置：
