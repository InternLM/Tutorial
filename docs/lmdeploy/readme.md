...
- 用 4GB 显存运行 8B 参数的大语言模型
- 搭建 OpenAI 兼容的 API 服务，让任意应用接入大模型能力
- 通过 W4A16 量化将模型显存占用降低 70%，推理速度提升 2 倍以上
...
以 InternLM3-8B-Instruct 在不同显卡上的表现为参考：
