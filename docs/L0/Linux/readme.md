...
## 目录：
- [Linux+InternStudio 关卡](#linuxinternstudio-关卡)
  - [一、InternStudio开发机介绍](#一internstudio开发机介绍)
  - [二、SSH及端口映射](#二ssh及端口映射)
    - [1. 什么是SSH？](#1-什么是ssh)
    - [2. 如何使用SSH远程连接开发机？](#2-如何使用ssh远程连接开发机)
      - [2.1 使用密码进行SSH远程连接](#21-使用密码进行ssh远程连接)
      - [2.2 配置SSH密钥进行SSH远程连接](#22-配置ssh密钥进行ssh远程连接)
      - [2.3 使用VScode进行SSH远程连接](#23-使用vscode进行ssh远程连接)
    - [3. 端口映射](#3-端口映射)
      - [3.1 什么是端口映射？](#31-什么是端口映射)
      - [3.2 如何进行端口映射？](#32-如何进行端口映射)
        - [3.2.1 使用 ssh 命令进行端口映射](#321-使用-ssh-命令进行端口映射)
        - [3.2.2 使用 vscode 进行端口映射](#322-使用-vscode-进行端口映射)
  - [三、Linux 基础命令](#三linux-基础命令)
    - [1. 文件管理](#1-文件管理)
      - [1.1 **touch**](#11-touch)
      - [1.2 **mkdir**](#12-mkdir)
      - [1.3 **cd**](#13-cd)
      - [1.4 **pwd**](#14-pwd)
      - [1.5 **cat**](#15-cat)
      - [1.6 **vi or vim**](#16-vi-or-vim)
      - [1.7 **cp 和 ln（重点）**](#17-cp-和-ln重点)
      - [1.8 **mv 和 rm**](#18-mv-和-rm)
      - [1.9 **find**](#19-find)
      - [1.10 **ls**](#110-ls)
      - [1.11 **sed**](#111-sed)
    - [2. 进程管理](#2-进程管理)
    - [3. **工具使用**](#3-工具使用)
  - [四、Conda和Shell介绍（附加）](#四conda和shell介绍附加)
    - [1. conda设置](#1-conda设置)
    - [2. conda环境管理](#2-conda环境管理)
      - [2.1 创建虚拟环境](#21-创建虚拟环境)
      - [2.2 查看有哪些虚拟环境](#22-查看有哪些虚拟环境)
      - [3.3 激活与退出虚拟环境](#33-激活与退出虚拟环境)
      - [3.4 删除与导出虚拟环境](#34-删除与导出虚拟环境)
    - [3. conda和pip](#3-conda和pip)
    - [4. studio-conda使用与Shell（扩展）](#4-studio-conda使用与shell扩展)
  - [常见问题](#常见问题)
    - [1. InternStudio 开发机的环境玩坏了，如何初始化开发机环境](#1-internstudio-开发机的环境玩坏了如何初始化开发机环境)
